...
➡️ **사용자는 직접 파일을 찾을 필요 없이 자연어로 원하는 정보를 조회할 수 있습니다.**

---

## 3) 코드 리팩토링 (LLM 기반 Code Transformer)

Agent는 검색된 Context와 사용자 요청을 기반으로 **코드를 자동 리팩토링**합니다.
...
➡️ **협업 프로젝트 온보딩 속도를 높이고 코드 품질을 유지하는 데 도움이 됩니다.**

---

# 🔍 Routing Signal Breakdown

Agent는 아래 세 가지 신호를 종합해 Routing을 결정합니다.
...
Routing Aggregator는 아래와 같은 **가중합(Weighted Sum)** 으로 신호를 통합합니다.
